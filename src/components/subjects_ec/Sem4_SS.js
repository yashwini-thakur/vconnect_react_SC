import React from "react";
function Sem4_SS() {
  return (
    <>
      <head>
        <link rel="stylesheet" href="ec_sem1_2.css" />
      </head>
      <body class="body_sub">
        <h2 class="subject_details">
          <b>
            <i>
              EC
              <br /> Sem:4 <br />
              Signal & System
            </i>
          </b>
        </h2>
        <div class="container_sub">
          <div class="card_sub">
            <div class="face_sub face1_sub">
              <div class="content_sub">
                <h3 class="h3_sub">Ch: 1</h3>
                <h3 class="h3_sub">Introduction to Signals & System</h3>
              </div>
            </div>
            <div class="face_sub face2_sub">
              <div class="content_sub">
                <p class="p_sub">You can get notes here:</p>
                <a href="https://drive.google.com/file/d/1Flm581205j7KwtcbKlDZyIJiUcSogjhF/view?usp=sharing">
                  NOTES
                </a>
              </div>
            </div>
          </div>
          <div class="card_sub">
            <div class="face_sub face1_sub">
              <div class="content_sub">
                <h3 class="h3_sub">Ch: 2</h3>
                <h3 class="h3_sub">
                  Behaviour of Continuous time (CT)& Discrete Time(DT) Linear
                  Time Invariant (LTI) System
                </h3>
              </div>
            </div>
            <div class="face_sub face2_sub">
              <div class="content_sub">
                <p class="p_sub">You can get notes here:</p>
                <a href="https://drive.google.com/file/d/1o9rjdvUBm6wsmErDVSB5pE1LKuDbbGz3/view?usp=sharing">
                  NOTES
                </a>
              </div>
            </div>
          </div>
          <div class="card_sub">
            <div class="face_sub face1_sub">
              <div class="content_sub">
                <h3 class="h3_sub">Ch: 3</h3>
                <h3 class="h3_sub">
                  Introduction to Fourier Series, Fourier Transform
                </h3>
              </div>
            </div>
            <div class="face_sub face2_sub">
              <div class="content_sub">
                <p class="p_sub">You can get notes here:</p>
                <a href="https://drive.google.com/file/d/1Mi3P3okM-uyBlxqTus5sEtHDJ18NJBkf/view?usp=sharing">
                  NOTES
                </a>
              </div>
            </div>
          </div>

          <div class="card_sub">
            <div class="face_sub face1_sub">
              <div class="content_sub">
                <h3 class="h3_sub">Ch: 4</h3>
                <h3 class="h3_sub">DTFT</h3>
              </div>
            </div>
            <div class="face_sub face2_sub">
              <div class="content_sub">
                <p class="p_sub">You can get notes here:</p>
                <a href="https://drive.google.com/file/d/1iEn6nC6NIpSzzAceVh-NkU1VMk8esrzk/view?usp=sharing">
                  NOTES
                </a>
              </div>
            </div>
          </div>
          <div class="card_sub">
            <div class="face_sub face1_sub">
              <div class="content_sub">
                <h3 class="h3_sub">Ch: 5</h3>
                <h3 class="h3_sub">Sampling & Reconstruction</h3>
              </div>
            </div>
            <div class="face_sub face2_sub">
              <div class="content_sub">
                <p class="p_sub">You can get notes here:</p>
                <a href="https://drive.google.com/file/d/1o9rjdvUBm6wsmErDVSB5pE1LKuDbbGz3/view?usp=sharing">
                  NOTES
                </a>
              </div>
            </div>
          </div>
        </div>
      </body>
    </>
  );
}
export default Sem4_SS;
